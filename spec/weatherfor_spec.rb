  ...
  context '#ApiConsultant', :vcr do
    context 'with correct token' do
      ...
      it '.avg_temp_in_days' do
        expect(request_api.avg_temp_in_days['04-17-2021']).not_to be_nil
        expect(request_api.avg_temp_in_days['04-18-2021']).not_to be_nil
        expect(request_api.avg_temp_in_days['04-19-2021']).not_to be_nil
        expect(request_api.avg_temp_in_days['04-20-2021']).not_to be_nil
        expect(request_api.avg_temp_in_days['04-21-2021']).not_to be_nil
      end

      it '.today_avg_temp' do
        expect(request_api.today_avg_temp).to be_a_kind_of(Integer)
        expect(request_api.today_avg_temp).not_to be_nil
      end

      it '.city_name' do
        expect(request_api.city_name).to be_a_kind_of(String)
        expect(request_api.city_name).not_to be_nil
        expect(request_api.city_name).to eq(city_name)
      end

      it '.current_temp_desc' do
        expect(request_api.current_temp_desc).to be_a_kind_of(String)
        expect(request_api.current_temp_desc).not_to be_nil
      end

      it '.current_date' do
        expect(request_api.current_date).to be_a_kind_of(String)
